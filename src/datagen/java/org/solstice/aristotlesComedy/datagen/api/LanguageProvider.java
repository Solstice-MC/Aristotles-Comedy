package org.solstice.aristotlesComedy.datagen.api;
...
public abstract class LanguageProvider implements DataProvider {
	...
	private final CompletableFuture<RegistryWrapper.WrapperLookup> lookupFuture;
	private final String modId;

	private final Map<String, String> data = new TreeMap<>();

	public LanguageProvider(ModContainer container, DataOutput output, CompletableFuture<RegistryWrapper.WrapperLookup> lookup) {
		this.output = output;
		this.lookupFuture = lookup;
		this.modId = container.getNamespace() + "-datagen";
	}
	...
	private CompletableFuture<?> run(DataWriter writer, RegistryWrapper.WrapperLookup lookup) {
		this.addTranslations(lookup);

		if (!data.isEmpty())
			return save(writer, this.output.resolvePath(DataOutput.OutputType.RESOURCE_PACK).resolve(this.modId).resolve("lang").resolve("en_us.json"));
	...
	public String getName() {
		return "Translations for mod: " + modId;
	}
